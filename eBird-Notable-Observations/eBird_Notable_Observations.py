...
r_notables_hotspots = requests.get(
    url_notables_hotspots, params={"key": api_key, "back": back_long, "hotspot": "True", "detail": "full"}
)

df_notables_hotspots = pd.DataFrame(r_notables_hotspots.json())

df_notables_hotspots["howMany"].fillna(value=1, inplace=True)

# Map hotspot observations with Plotly
...
fig_hotspots = px.scatter_mapbox(
    ...
    title=f"Notable Observations at Hotspots in {state} in the last {back_long} days (Today's date: {today})",
